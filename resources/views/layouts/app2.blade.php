...
<head>
    ...
    <title>@if (@isset($search)) {{$search}} -  @endif {{str_replace('MaxPreps',$seo->site_name,@$data->first()->kategori)}}</title>
	<link rel="canonical" href="{{@url()->full()}}" />
    <meta http-equiv="X-UA-Compatible" content="IE=edge" />
    <meta name="description" content="@if(@isset($item2->title2)){{$item2->title2}} - {{@$seo->site_name}} @else @if (@isset($search)) {{$search}} -  @endif{{@$seo->description}} - {{@$seo->site_name}} @endif" />
    ...
    <meta property="og:description" content="@if(@isset($item2->title2)){{$item2->title2}} - {{@$seo->site_name}} @else @if (@isset($search)) {{$search}} -  @endif{{@$seo->description}} - {{@$seo->site_name}} @endif" /> 
        ...
        <nav class="navbar navbar-expand-md navbar-light bg-light shadow-sm">
            <div class="container">
                <a class="navbar-brand" href="{{ url('/') }}">
                    <img style="height: 50px" src="{{url("logo2.png")}}" alt="high school sport logo" class="img-fluid">
                </a>
